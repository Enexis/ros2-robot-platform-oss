...
transformer_rd2wgs = Transformer.from_crs("EPSG:28992", "EPSG:4326", always_xy=True)
transformer_wgs2rd = Transformer.from_crs("EPSG:4326", "EPSG:28992", always_xy=True)

def rd_to_wgs84(x: float, y: float):
    # EPSG:28992 (Amersfoort / RD New) -> EPSG:4326 (WGS84 lat/lon)
    lon, lat = transformer_rd2wgs.transform(x, y)
    return lat, lon

def wgs84_to_rd(latitude: float, longitude: float):
    # EPSG:4326 (WGS84 lat/lon) -> EPSG:28992 (Amersfoort / RD New)
    x, y = transformer_wgs2rd.transform(longitude, latitude)
    return x, y
