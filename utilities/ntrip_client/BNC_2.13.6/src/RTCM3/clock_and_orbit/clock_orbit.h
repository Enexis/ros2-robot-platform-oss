...
public:
  ...
  typedef unsigned int PhaseBiasExtType;
  PhaseBiasExtType PBEXTTYPE_BASE;
  PhaseBiasExtType PBEXTTYPE_GPS;
  PhaseBiasExtType PBEXTTYPE_GLONASS;
  PhaseBiasExtType PBEXTTYPE_GALILEO;
...
#define COBOFS_MAXNUM 7
  ...
  virtual size_t MakePhaseBias(const struct PhaseBias *b, CodeBiasType type,
      int moremessagesfollow, char *buffer, size_t size) = 0;
  virtual size_t MakeVTEC(const struct VTEC *v,
      int moremessagesfollow, char *buffer, size_t size) = 0;
