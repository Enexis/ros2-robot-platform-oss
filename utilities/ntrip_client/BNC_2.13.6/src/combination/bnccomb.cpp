...
const double sigP_clkSat   =  100.0; //*) White noise may be set to something between a few centimeters and tens of meters.
...
bncComb::bncComb() : _ephUser(true) {
  ...
  _cmbSampl = settings.value("cmbSampl").toString().split("sec").first().toInt();
  if (_cmbSampl <= 0) {
    _cmbSampl = 5;
  }
...
void bncComb::dumpResults(bncTime epoTime, QMap<QString, cmbCorr*>& masterCorr) {
  ...
  while (it.hasNext()) {
    ...
    if (corr->_satCodeBias._bias.size()) {
      ...
      for (unsigned ii = 0; ii < satCodeBias._bias.size(); ii++) {
        ...
        if (!frqCodeBias._rnxType2ch.empty()) {
          hlp = QString().asprintf(" %s%12.6f",
                                   frqCodeBias._rnxType2ch.c_str(),
                                   frqCodeBias._value);
          outLines += hlp;
          hlp.clear();
        }
    ...
    // PHASE BIASES
    /* can be added as soon as yaw agles from the ACs have useful values
    t_satPhaseBias satPhaseBias(corr->_satPhaseBias);
    satPhaseBias._staID = "INTERNAL";
    satPhaseBias._yaw = corr->_satYawAngle;
    satPhaseBiasList.push_back(satPhaseBias);
    hlp = QString().asprintf(" YawAngle  %1.4f",  satPhaseBias._yaw);
    outLines += hlp;
    hlp.clear();
    hlp = QString().asprintf(" PhaseBias %2lu", satPhaseBias._bias.size());
    outLines += hlp;
    hlp.clear();
    for (unsigned ii = 0; ii < satPhaseBias._bias.size(); ii++) {
      ...
      if (!frqPhaseBias._rnxType2ch.empty()) {
        hlp = QString().asprintf(" %s%10.6f%3d%3d%3d",
                                 frqPhaseBias._rnxType2ch.c_str(),
                                 frqPhaseBias._value,
                                 frqPhaseBias._fixIndicator,
...
void bncComb::slotProviderIDChanged(QString mountPoint) {
  ...
  while (icAC.hasNext()) {
    ...
    if (AC->mountPoint == mountPoint) {
      acName = AC->name;
      out << "SSR Provider or Service changed: AC " << AC->name.toLatin1().data() << " "
