...
void bncGetThread::initialize() {
  ...
  if (nmeaPort != 0) {
    ...
    if (!_nmeaServer->listen(QHostAddress::LocalHost, nmeaPort)) {
  ...
  if (!_staID.isEmpty()
    ...
    _serialPort = new QextSerialPort(portString);
    ...
    if (!_serialPort->isOpen()) {
      ...
      emit(newMessage(_staID + ": Serial port " + portString.toLatin1()
           + "is connected: " + _serialPort->errorString().toLatin1(), true));
