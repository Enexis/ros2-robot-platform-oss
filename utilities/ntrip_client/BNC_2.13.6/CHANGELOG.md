...
- FIXED: Bug in IGS SSR Epoch Time, which is defined as follows: Full seconds since the beginning of the week of continuous time scale with no offset from GPS, Galileo, QZSS, SBAS, UTC leap seconds from GLONASS, -14 s offset from BDS
...
## 2.13.3 (2025-07-18)
- FIXED: Bug in RINEX QC (RINEX version 2)
- FIXED: Bug in RTNET upload
- ADDED: Orbit and attitude correction in combination approach
- ADDED: RTCM message 1230 (on-change) and the size of each RTCM message in scanRTCM()
- CHANGED: Bind all outPorts to localHost
