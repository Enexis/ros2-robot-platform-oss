...
The docker-compose will build the BNC program using a build container provided by BKG, it is set to debian:bookworm by default but this can be changed depending on the platform required. From that image it is then copied to a small ntrip_client image for running.
